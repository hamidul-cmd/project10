import React from "react";

function TestimonialsCird({ img, name, comment }) {
  return (
    <>
      <div className="p-7.5 bg-white rounded-10 border-2 border-gray-15 shadow-black space-y-5 text-center w-[358px] min-w-[358px] flex-shrink-0 ll:w-[320px] ll:min-w-[320px] xll:w-[358px] xll:min-w-[358px] 3xl:w-[400px] 3xl:min-w-[400px] h-full">
        <div className="space-y-2.5">
          <img
            src={img}
            alt=""
            className="block h-[66px] w-[66px] 3xl:h-20 3xl:w-20 m-auto select-none"
          />
          <h3 className="text-xl font-semibold l-150 font-outfit text-gray-20 select-none">
            {name}
          </h3>
        </div>
        <div className="flex justify-center gap-1 select-none">
          <div>
            <svg
              className="h-5 w-5 3xl:h-6 3xl:w-6"
              viewBox="0 0 16 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M6.27465 1.9429C7.04734 0.624933 8.95266 0.624932 9.72535 1.9429L10.8023 3.7798C11.0843 4.26076 11.5542 4.6022 12.0988 4.72175L14.1785 5.17833C15.6708 5.50593 16.2596 7.31799 15.2449 8.46014L13.8307 10.052C13.4604 10.4688 13.2809 11.0212 13.3354 11.5761L13.5439 13.6952C13.6935 15.2156 12.152 16.3355 10.7522 15.7234L8.80127 14.8704C8.29044 14.647 7.70956 14.647 7.19873 14.8704L5.24777 15.7234C3.84797 16.3355 2.30654 15.2156 2.45611 13.6952L2.66456 11.5761C2.71914 11.0212 2.53963 10.4688 2.16934 10.052L0.755137 8.46014C-0.259549 7.31799 0.329225 5.50593 1.82146 5.17833L3.90124 4.72175C4.4458 4.6022 4.91575 4.26076 5.19773 3.7798L6.27465 1.9429Z"
                fill="#FF8D4D"
              />
            </svg>
          </div>
          <div>
            <svg
              className="h-5 w-5 3xl:h-6 3xl:w-6"
              viewBox="0 0 16 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M6.27465 1.9429C7.04734 0.624933 8.95266 0.624932 9.72535 1.9429L10.8023 3.7798C11.0843 4.26076 11.5542 4.6022 12.0988 4.72175L14.1785 5.17833C15.6708 5.50593 16.2596 7.31799 15.2449 8.46014L13.8307 10.052C13.4604 10.4688 13.2809 11.0212 13.3354 11.5761L13.5439 13.6952C13.6935 15.2156 12.152 16.3355 10.7522 15.7234L8.80127 14.8704C8.29044 14.647 7.70956 14.647 7.19873 14.8704L5.24777 15.7234C3.84797 16.3355 2.30654 15.2156 2.45611 13.6952L2.66456 11.5761C2.71914 11.0212 2.53963 10.4688 2.16934 10.052L0.755137 8.46014C-0.259549 7.31799 0.329225 5.50593 1.82146 5.17833L3.90124 4.72175C4.4458 4.6022 4.91575 4.26076 5.19773 3.7798L6.27465 1.9429Z"
                fill="#FF8D4D"
              />
            </svg>
          </div>
          <div>
            <svg
              className="h-5 w-5 3xl:h-6 3xl:w-6"
              viewBox="0 0 16 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M6.27465 1.9429C7.04734 0.624933 8.95266 0.624932 9.72535 1.9429L10.8023 3.7798C11.0843 4.26076 11.5542 4.6022 12.0988 4.72175L14.1785 5.17833C15.6708 5.50593 16.2596 7.31799 15.2449 8.46014L13.8307 10.052C13.4604 10.4688 13.2809 11.0212 13.3354 11.5761L13.5439 13.6952C13.6935 15.2156 12.152 16.3355 10.7522 15.7234L8.80127 14.8704C8.29044 14.647 7.70956 14.647 7.19873 14.8704L5.24777 15.7234C3.84797 16.3355 2.30654 15.2156 2.45611 13.6952L2.66456 11.5761C2.71914 11.0212 2.53963 10.4688 2.16934 10.052L0.755137 8.46014C-0.259549 7.31799 0.329225 5.50593 1.82146 5.17833L3.90124 4.72175C4.4458 4.6022 4.91575 4.26076 5.19773 3.7798L6.27465 1.9429Z"
                fill="#FF8D4D"
              />
            </svg>
          </div>
          <div>
            <svg
              className="h-5 w-5 3xl:h-6 3xl:w-6"
              viewBox="0 0 16 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M6.27465 1.9429C7.04734 0.624933 8.95266 0.624932 9.72535 1.9429L10.8023 3.7798C11.0843 4.26076 11.5542 4.6022 12.0988 4.72175L14.1785 5.17833C15.6708 5.50593 16.2596 7.31799 15.2449 8.46014L13.8307 10.052C13.4604 10.4688 13.2809 11.0212 13.3354 11.5761L13.5439 13.6952C13.6935 15.2156 12.152 16.3355 10.7522 15.7234L8.80127 14.8704C8.29044 14.647 7.70956 14.647 7.19873 14.8704L5.24777 15.7234C3.84797 16.3355 2.30654 15.2156 2.45611 13.6952L2.66456 11.5761C2.71914 11.0212 2.53963 10.4688 2.16934 10.052L0.755137 8.46014C-0.259549 7.31799 0.329225 5.50593 1.82146 5.17833L3.90124 4.72175C4.4458 4.6022 4.91575 4.26076 5.19773 3.7798L6.27465 1.9429Z"
                fill="#FF8D4D"
              />
            </svg>
          </div>
          <div>
            <svg
              className="h-5 w-5 3xl:h-6 3xl:w-6"
              viewBox="0 0 16 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M6.27465 1.9429C7.04734 0.624933 8.95266 0.624932 9.72535 1.9429L10.8023 3.7798C11.0843 4.26076 11.5542 4.6022 12.0988 4.72175L14.1785 5.17833C15.6708 5.50593 16.2596 7.31799 15.2449 8.46014L13.8307 10.052C13.4604 10.4688 13.2809 11.0212 13.3354 11.5761L13.5439 13.6952C13.6935 15.2156 12.152 16.3355 10.7522 15.7234L8.80127 14.8704C8.29044 14.647 7.70956 14.647 7.19873 14.8704L5.24777 15.7234C3.84797 16.3355 2.30654 15.2156 2.45611 13.6952L2.66456 11.5761C2.71914 11.0212 2.53963 10.4688 2.16934 10.052L0.755137 8.46014C-0.259549 7.31799 0.329225 5.50593 1.82146 5.17833L3.90124 4.72175C4.4458 4.6022 4.91575 4.26076 5.19773 3.7798L6.27465 1.9429Z"
                fill="#FF8D4D"
              />
            </svg>
          </div>
        </div>
        <p className="text-base font-medium l-150 text-gray-20 font-outfit select-none">
          {comment}
        </p>
      </div>
    </>
  );
}

export default TestimonialsCird;
